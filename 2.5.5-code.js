...
(function () {    
  ...
  elements.forEach(el => {
    ...
    svg.style.top = `${center.top - 22}px`;
    svg.style.left = `${center.left - 22}px`;
  ...
  centers.forEach((item1, index) => {
    centers.slice(index + 1).forEach(item2 => {
      if (Math.sqrt(Math.pow(item2.center.left - item1.center.left, 2) + Math.pow(item2.center.top - item1.center.top, 2)) < 44) {
